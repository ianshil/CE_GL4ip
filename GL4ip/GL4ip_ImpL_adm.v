...
- intros A B Γ0 Γ1 C hei eq D1. subst.
  ...
  * inversion H ; subst.
  ...
  (* BotL *)
  * inversion H. subst. assert (InT (Bot V) (Γ0 ++ A → B :: Γ1)). assert (InT (Bot V) (Γ0 ++ Γ1)).
    ...
    assert (BotLRule [] (x ++ Bot V :: x0, C)). apply BotLRule_I. apply BotL in H0.
    pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
    (ps:=[]) (x ++ Bot V :: x0, C) H0 DersNilF). auto.
   (* AndR *)
