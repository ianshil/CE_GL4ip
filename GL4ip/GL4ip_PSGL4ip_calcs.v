...
Inductive MPropF (W : Set) : Type :=
 | Var : W -> MPropF W
 | Bot : MPropF W
 | And : MPropF W -> MPropF W -> MPropF W
 | Or : MPropF W -> MPropF W -> MPropF W
...
Fixpoint weight_form (A : MPropF V) : nat :=
match A with
 | # P => 1
 | Bot _ => 1
...
Definition is_atomicT (A : MPropF V) : Type :=
                  (existsT2 (P : V), A = # P) + (A = Bot V).

Definition is_boxedT (A : MPropF V) : Type :=
                  exists (B : MPropF V), A = Box B.

Definition is_boxedT2 (A : MPropF V) : Type :=
                  existsT2 (B : MPropF V), A = Box B.

Definition is_primeT (A : MPropF V) : Type :=
...
Definition is_Atomic (Γ : list (MPropF V)) : Prop :=
    forall (A : MPropF V), (In A Γ) -> ((exists (P : V), A = # P) \/ (A = Bot V)).

Definition is_Boxed_list (Γ : list (MPropF V)) : Prop :=
    forall (A : MPropF V), (In A Γ) -> (exists (B : MPropF V), A = Box B).

Definition is_Prime (Γ : list (MPropF V)) : Prop :=
...
Definition unBox_formula (A : MPropF V) : MPropF V :=
  match A with
              | # P => # P
              | Bot _ => Bot _
              | And A B => And A B
...
Fixpoint XBoxed_list (Γ : list (MPropF V)) : list (MPropF V):=
  match Γ with
  | [ ] => [ ]
...
Inductive GLRRule : rlsT (prod (list (MPropF V)) (MPropF V)) :=
...
Inductive GL4ip_rules : rlsT  (prod (list (MPropF V)) (MPropF V)) :=
...
(* To make it terminating, I need to constrain all rules not to be applied on identities of all complexity. *)

Inductive PSGL4ip_rules : rlsT  (prod (list (MPropF V)) (MPropF V)) :=
