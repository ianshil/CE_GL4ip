...
- intros hei. intros prem RA. inversion RA. subst.
  ...
  * inversion H.
  (* BotL *)
  * inversion H. subst. assert (InT (Bot V) (Γ0 ++ A :: Γ1)). assert (InT (Bot V) (Γ0 ++ Γ1)).
    ...
    assert (BotLRule [] (x ++ Bot V :: x0, B)). apply BotLRule_I. apply BotL in H0.
    pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
    (ps:=[]) (x ++ Bot V :: x0, B) H0 DersNilF). exists d0.
