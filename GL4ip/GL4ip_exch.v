...
Inductive list_exch_L : relationT (list (MPropF V) * (MPropF V))  :=
...
Lemma list_exch_L_permR : forall s se, (*  Useless I reckon. *)
    (@list_exch_L s se) ->
      (forall Γ A, s = (Γ, A) ->
      (existsT2 eΓ, se = (eΓ, A))).
Proof.
intros s se exch. induction exch. intros Γ A0 E.
...
destruct H.
- destruct s. destruct s. rewrite e.
  assert (E: (x ++ Bot V :: x0) ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4 =
  x ++ Bot V :: x0 ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4). repeat rewrite <- app_assoc. auto.
  rewrite E. apply BotLRule_I.
- destruct s.
  + destruct s. destruct s. rewrite e.
    assert (E: Γ0 ++ Γ1 ++ Γ2 ++ (x ++ Bot V :: x0) ++ Γ4 =
    Γ0 ++ Γ1 ++ Γ2 ++ x ++ Bot V :: x0 ++ Γ4). repeat rewrite <- app_assoc. auto. rewrite E.
    assert (E1: Γ0 ++ Γ1 ++ Γ2 ++ x ++ Bot V :: x0 ++ Γ4 =
    (Γ0 ++ Γ1 ++ Γ2 ++ x) ++ Bot V :: x0 ++ Γ4). repeat rewrite <- app_assoc. auto.
    rewrite E1. apply BotLRule_I.
  + destruct s.
    * destruct s. destruct s. rewrite e.
      assert (E: Γ0 ++ Γ1 ++ (x ++ Bot V :: x0) ++ Γ3 ++ Γ4 =
      (Γ0 ++ Γ1 ++ x) ++ Bot V :: x0 ++ Γ3 ++ Γ4). repeat rewrite <- app_assoc. auto.
      rewrite E. apply BotLRule_I.
    * destruct s.
      { destruct s. destruct s. rewrite e.
        assert (E: Γ0 ++ (x ++ Bot V :: x0) ++ Γ2 ++ Γ3 ++ Γ4 =
        (Γ0 ++ x) ++ Bot V :: x0 ++ Γ2 ++ Γ3 ++ Γ4). repeat rewrite <- app_assoc. auto.
        rewrite E. apply BotLRule_I. }
      { destruct s. destruct s. rewrite e.
        assert (E: Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ x ++ Bot V :: x0 =
        (Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ x) ++ Bot V :: x0). repeat rewrite <- app_assoc. auto.
        rewrite E. apply BotLRule_I. }
Qed.
...
pose (strong_inductionT (fun (x:nat) => forall (s : list (MPropF V) * (MPropF V))
  (D0 : derrec GL4ip_rules (fun _ : list (MPropF V) * (MPropF V) => False) s),
x = derrec_height D0 ->
(forall se,
(list_exch_L s se) ->
(existsT2
  D1 : derrec GL4ip_rules
         (fun _ : list (MPropF V) * (MPropF V) => False) se,
  derrec_height D1 <= x)))).
...
- intros hei se exch. inversion exch.
  assert (DersNil: dersrec GL4ip_rules (fun _ : list (MPropF V) * (MPropF V) => False) []).
  ...
  * inversion H1. subst. inversion H5. subst. simpl.
  ...
  (* BotL *)
  * inversion H1. subst. inversion H5. subst. simpl.
    assert (In (Bot V) (Γ0 ++ Γ3 ++ Γ2 ++ Γ1 ++ Γ4)). assert (In (Bot V) (Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4)).
    rewrite <- H0. apply in_or_app. right. apply in_eq. apply in_app_or in H. apply in_or_app.
    destruct H. auto. apply in_app_or in H. right. apply in_or_app. destruct H. right. apply in_or_app.
    right. apply in_or_app. auto. apply in_app_or in H. destruct H. right. apply in_or_app. auto.
    apply in_app_or in H. destruct H. auto. right. apply in_or_app. right. apply in_or_app. auto.
    apply in_splitT in H. destruct H. destruct s. rewrite e.
    assert (BotLRule [] (x ++ Bot V :: x0, A)). apply BotLRule_I. apply BotL in H.
    pose (derI (rules:=GL4ip_rules) (prems:=fun _ : list (MPropF V) * (MPropF V) => False)
    (ps:=[]) (x ++ Bot V :: x0, A) H DersNil). exists d0. simpl. rewrite dersrec_height_nil.
    lia. reflexivity.
 (* AndR *)
