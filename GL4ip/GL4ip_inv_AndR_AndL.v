...
{ intros prem RA. inversion RA. subst.
  ...
  * inversion H. subst. assert (InT # P (Γ0 ++ And A B :: Γ1)).
  ...
  (* BotL *)
  * inversion H. subst. assert (InT (Bot V) (Γ0 ++ And A B :: Γ1)).
    rewrite <- H2. apply InT_or_app. right. apply InT_eq. assert (InT (Bot V) (Γ0 ++ A :: B :: Γ1)).
    apply InT_app_or in H0. destruct H0. apply InT_or_app. auto. apply InT_or_app. right.
    ...
    pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
    (ps:=[]) (x ++ Bot V :: x0, C) H1 DersNilF). exists d0.
    simpl. rewrite dersrec_height_nil. lia. reflexivity.
   (* AndR *)
...
{ intros prem1 prem2 RA. inversion RA. subst.
  ...
  * inversion H. subst. simpl.
    assert (BotLRule [] (Γ0 ++ Bot V :: Γ1, A)).
    apply BotLRule_I. apply BotL in H0.
    pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
    (ps:=[]) (Γ0 ++ Bot V :: Γ1, A) H0 DersNilF). exists d0.
    assert (BotLRule [] (Γ0 ++ Bot V :: Γ1, B)).
    apply BotLRule_I. apply BotL in H1.
    pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
    (ps:=[]) (Γ0 ++ Bot V :: Γ1, B) H1 DersNilF). exists d1.
