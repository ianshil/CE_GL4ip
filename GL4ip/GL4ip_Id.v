...
Lemma Id_all_form0 : forall n (A : MPropF V) Γ0 Γ1,
          (n = weight_form A) ->
...
- intros. assert (IdPRule [] (Γ0 ++ # v :: Γ1, # v)). apply IdPRule_I. apply IdP in H0.
  apply derI with (rules:=GL4ip_rules) (prems:=fun _ : (prod (list (MPropF V)) (MPropF V)) => False) (ps:=[]).
  assumption. apply dersrec_nil.
- intros. assert (BotLRule [] (Γ0 ++ Bot V :: Γ1, Bot V)). apply BotLRule_I. apply BotL in H0.
  apply derI with (rules:=GL4ip_rules) (prems:=fun _ : (prod (list (MPropF V)) (MPropF V)) => False) (ps:=[]).
  assumption. apply dersrec_nil.
- intros.  assert (AndLRule [(Γ0 ++ A1 :: A2 :: Γ1, And A1 A2)]  (Γ0 ++ And A1 A2 :: Γ1, And A1 A2)).
  ...
  apply derI with (rules:=GL4ip_rules) (prems:=fun _ : (prod (list (MPropF V)) (MPropF V)) => False) (ps:=[(Γ0 ++ A1 :: A2 :: Γ1, And A1 A2)]) ; auto.
  apply dlCons. 2 : apply dersrec_nil.
  apply derI with (rules:=GL4ip_rules) (prems:=fun _ : (prod (list (MPropF V)) (MPropF V)) => False) (ps:=[(Γ0 ++ A1 :: A2 :: Γ1, A1); (Γ0 ++ A1 :: A2 :: Γ1, A2)]).
  assumption. apply dlCons. auto. apply dlCons. auto. apply dersrec_nil.
- intros.
  apply derI with (rules:=GL4ip_rules) (prems:=fun _ : (prod (list (MPropF V)) (MPropF V)) => False)
  ...
  apply dersrec_nil.
- intros. apply derI with (rules:=GL4ip_rules) (prems:=fun _ : (prod (list (MPropF V)) (MPropF V)) => False)
...
Lemma Id_all_form : forall (A : MPropF V) Γ0 Γ1,
          derrec GL4ip_rules (fun _ => False) (Γ0 ++ A :: Γ1, A).
Proof.
intros.
