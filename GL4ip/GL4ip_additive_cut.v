...
apply dersrec_nil.
assert (DersNilT: dersrec GL4ip_rules (fun _ : (list (MPropF V)) *(MPropF V) => True) []).
apply dersrec_nil.
...
- inversion H1. subst. inversion X0. subst. clear X4. clear X0. inversion X1. simpl.
      ...
      { inversion H. subst. rewrite H2. assert (J0 : InT (Bot V) (Γ0 ++ (A0 → B) :: Γ1)). rewrite <- H6. apply InT_or_app.
        ...
        - apply InT_split in i. destruct i. destruct s. subst. rewrite <- app_assoc.
          assert (BotLRule [] (x ++ (⊥ V :: x0) ++ Γ1, C)). apply BotLRule_I. apply BotL in H0.
          pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
          (ps:=[]) (x ++ (⊥ V :: x0) ++ Γ1, C) H0 DersNilF). assumption.
        - inversion i.
          * inversion H3.
          * apply InT_split in H3. destruct H3. destruct s. subst. rewrite app_assoc.
            assert (BotLRule [] ((Γ0 ++ x) ++ ⊥ V :: x0, C)). apply BotLRule_I. apply BotL in H0.
            pose (derI (rules:=GL4ip_rules) (prems:=fun _ : (list (MPropF V)) *(MPropF V) => False)
            (ps:=[]) ((Γ0 ++ x) ++ ⊥ V :: x0, C) H0 DersNilF). assumption. }
    (* Right rule is AndR *)
    ...
    { inversion H. subst. inversion X2. subst. clear X4. clear X2. apply list_split_form in H6. destruct H6 ; subst. destruct s.
      - repeat destruct p. inversion e0. subst. rewrite H2.
        ...
        assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) ([] ++ Γ0 ++ Γ1, A → B0 → B)).
    ...
    { inversion H. subst. inversion X2. subst. clear X4. clear X2. apply list_split_form in H6. destruct H6 ; subst. destruct s.
      - repeat destruct p. inversion e0. subst. rewrite H2.
        ...
        assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) ([] ++ Γ0 ++ Γ5 ++ Γ6, A → B)).
        apply derI with (ps:=[([] ++ A :: Γ0 ++ Γ5 ++ Γ6, B)]). apply ImpR. apply ImpRRule_I. apply dlCons ; auto. simpl in X2.
        assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) ([] ++ Γ0 ++ Γ5 ++ Γ6, B0 → B)).
    ...
    { inversion H. subst. inversion X2. subst. inversion X4. subst. clear X6. clear X4. clear X2.
      ...
      - repeat destruct p. inversion e0. subst. rewrite H2.
        ...
        assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) ([] ++ Γ0 ++ Γ1, B0 → B)).
...
- inversion X3. subst. inversion X1.
    ...
    { inversion X5. subst. inversion X0. subst. clear X8. clear X0. inversion X2. subst. inversion X8. subst. clear X10.
      ...
      - repeat destruct s ; repeat destruct p ; subst.
        ...
        assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) (XBoxed_list (x6 ++ x3 ++ x2) ++ [Box A], A)).
        { assert (x = x6). { apply nobox_gen_ext_injective with (l:=Γ0) ; auto. intro ; intros. apply H4. apply in_or_app. left.
          ...
          assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) (XBoxed_list x6 ++ XBoxed_list (l ++ x5) ++ [Box A], Box A0)).
          { assert (XBoxed_list x6 ++ XBoxed_list (l ++ x5) ++ [Box A] = XBoxed_list (x6 ++ l ++ x5) ++ [Box A]). repeat rewrite XBox_app_distrib.
            ...
            assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) (x6 ++ l ++ x5,  Box A0)).
      ...
      - repeat destruct s ; repeat destruct p ; subst.
        ...
        assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) (XBoxed_list (x4 ++ x5 ++ x0) ++ [Box A], A)).
        { assert (J22: In (XBoxed_list (x4 ++ x5 ++ x0) ++ [Box A], A) [(XBoxed_list (x4 ++ x5 ++ x0) ++ [Box A], A); (Γ2 ++ B :: x3 ++ Γ1, C)]). apply in_eq.
          ...
          assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) (XBoxed_list (x4 ++ x5) ++ XBoxed_list x0 ++ [Box A], Box A0)).
          { assert (XBoxed_list (x4 ++ x5) ++ XBoxed_list x0 ++ [Box A] = XBoxed_list (x4 ++ x5 ++ x0) ++ [Box A]). repeat rewrite XBox_app_distrib.
            repeat rewrite <- app_assoc. auto. rewrite H. clear H.
            assert (derrec GL4ip_rules (fun _ : list (MPropF V) * MPropF V => False) (x4 ++ x5 ++ x0,  Box A0)).
    ...
    { inversion X5. subst. inversion X0. subst. clear X8. inversion X2. subst. clear X9.
      ...
      - subst.
        ...
        assert (X7': derrec GL4ip_rules (fun _ : (list (MPropF V)) *(MPropF V) => False) ((XBoxed_list (x ++ x0) ++ [Box A0]) ++ [], A0)).
